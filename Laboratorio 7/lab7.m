...
figure;imshow(uint8(qualid50));
imwrite(uint8(qualid50),'qualid50.png');
entropia_imagem = entropy(img);
entropia_qualid = entropy(qualid50);
fun3 =@(X) round(X.*QuantT);
recuperada = 8*blkproc(qualid50,[8 8],fun3);
recuperada = blkproc(recuperada,[8 8],@idct2);
figure;imshow(uint8(recuperada));
imwrite(uint8(recuperada),'recuperada.png');
