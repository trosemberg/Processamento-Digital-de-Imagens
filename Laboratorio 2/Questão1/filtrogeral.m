...
function [novaimg] = filtrogeral (imagem, filter)
  ...
  imgtemp = zeros(tamimg(1) + 2*tamfil(1) , tamimg(2) + 2*tamfil(2), tamimg(3));%cria uma imagem temporaria com o tamanho da imagem mais duas vezes o tamanho do filtrogeral de altura e largura

  imgtemp (tamfil(1)+1:tamimg(1) + tamfil(1),tamfil(2)+1:tamimg(2) + tamfil(2), 1:tamimg(3))...
  =img(1:tamimg(1),1:tamimg(2),1:tamimg(3)); %preenche a imagem temporaria para que tenha uma borda ao redor da imagem original, sendo tal borda do tamanho do filtrogeral
      ...
      for(coluna = tamfil(2)+1:1:tamimg(2) + tamfil(2)) %for para varrer as colunas
        for(x=1:1:tamfil(1))%for para varrer o tamanho do filtrogeral em linha
          for(y=1:1:tamfil(2)) %for para varrer o tamanho do filtrogeral em coluna
            novaimg(linha-tamfil(1),coluna-tamfil(2),prof) = novaimg(linha-tamfil(1),coluna-tamfil(2),prof)+imgtemp(linha-ceil(tamfil(1)/2)+x,coluna-ceil(tamfil(2)/2)+y,prof)* filter(x,y);
          end
        end
      end
    end
  ...
  imshow(novaimg);
  imwrite(novaimg,"imagemfiltrada.png")
endfunction
